...
author: "Victoria"
date: "created 4 October 2021, updated 6 October 2022"
output:
  html_document:
    toc: yes
    toc_float: yes
...
As far as I can see, the data looks fine. It is collected from a lot of different places, but it looks as if this is done to 'fill the gaps'. 
...
Rate means a part of something. Using rates is better because it relates to how many e.g. inhabitants a country has and enables us to compare. Maybe one country only has 1 million inhabitants but 100 homicides. A country with 10 million inhabitants but also 100 homicides would have a lower rate, compared to just looking at the number itself.
...
Western_Europe %>%
    ...
    geom_line() +
    labs(title = "Homicides in Western Europe over time",
       colour = "Region",
       x = "Year",
...
```{r eval = FALSE}
king_data <- read.csv2("data/kings_dk_data.csv")
# I find the duration below
king_data$Duration <- king_data$End_date-king_data$Start_date 
king_data$Midyear <- (king_data$End_date - (king_data$End_date-king_data$Start_date)/2) #Midyear column
  king_data %>% 
  ggplot(aes(x = Midyear, y = Duration)) +
  geom_smooth() +
  labs(title = "Danish monarchs reign over time",
       x = "Year",
       y = "Duration of reign")
```
The duration of reign seems to increase over time. This is the opposite trend of the homicides.
...
2) Rmarkdown: 
  ...
  - automatically generate a `timestamp` to show when the document was last updated.
  I think I did it correctly :)
  (Hint: check the Rmarkdown episode in our Data Carpentry tutorial)
...
If a country's civilization is measured by how many homocides per 100,000 individuals, then yes we are more civilized. The visualisations show a trend of less homicides since 1300 in all of the regions. Looking at the visualisation of the reign of Danish monarchs, they also reign for longer periods of time. This is a guess, but it could show that the country has become more stable with less wars and controversies, but might also be because we generally live longer today. 